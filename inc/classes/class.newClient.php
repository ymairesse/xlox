...
class NewClient
{
    ...
    public static function autoSaveClient($form)
    {
        ...
        $typeClient = isset($form['typeClient']) ? $form['typeClient'] : 'prive';
        ...
        $sql .= 'droits = :droits, rgpd = :rgpd, typeClient = :typeClient ';
        if ($pwd != null) {
            $sql .= ', md5passwd = :pwd ';
        }
        ...
        $civilite = self::getCivilite($civilite);

        $quidam = sprintf('%s %s %s', $civilite, $prenom, $nom);
